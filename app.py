...
def generate_content(client, model, text, language, meta_type):
    prompt = f"You are a specialized assistant trained to craft the optimal {meta_type} for SEO in {language}. Your task is to produce content that is human-like, unique, and effective for boosting Click-Through Rate (CTR). You will be given a combination of Title, Meta Description, H1 and target keyword. It's possible that one or more of these inputs might be 'None' or that the page doesn't exist. In such cases, ignore these inputs and create something new based on the available information. Respond in the exact format: '[your {meta_type} here]' without any quotation or annotation marks around the content. Your response should be in {language}. Adapt your language style to match the tone of the current Title, Meta Description and H1. Do not include any notes, explanations, or additional information. Focus solely on generating the {meta_type} for the target keyword. Try to fit in the keyword as natural as possible. Give out the full text within the max _tokens limit"
